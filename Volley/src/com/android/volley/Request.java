...
/**
 * 代表网络请求的抽象类
 * 我们通过构建�?��Request类的非抽象子�?StringRequest、JsonRequest、ImageRequest或自定义)对象�? * 并将其加入到·RequestQueue·中来完成�?��网络请求操作�? * Volley 支持 8 �?Http 请求方式 GET, POST, PUT, DELETE, HEAD, OPTIONS, TRACE, PATCH
 * Request 类中包含了请�?url，请求请求方式，请求 Header，请�?Body，请求的优先级等信息�? * 由于是抽象类，所以必须重写的两个类：
 * abstract protected Response<T> parseNetworkResponse(NetworkResponse response);
 * 子类重写此方法，将网络返回的原生字节内容，转换成合�?的类型�?此方法会在工作线程中被调�? * abstract protected void deliverResponse(T response);
 * 子类重写此方法，将解析成合�?类型的内容传递给它们的监听回调�?
 * 以下两个方法也经常会被重�?
     public byte[] getBody()
     重写此方法，可以构建用于 POST、PUT、PATCH 请求方式�?Body 内容�?
     protected Map<String, String> getParams()
     在上面getBody函数没有被重写情况下，此方法的返回�?会被 key、value 分别编码后拼装起来转换为字节码作�?Body 内容�? */
/**
